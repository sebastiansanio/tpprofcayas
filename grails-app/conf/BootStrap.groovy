    ...
    def init = { servletContext ->
		...
		if(Environment.current != Environment.PRODUCTION) {
			User.withTransaction{		
				...
				alertType.save(flush:true)
				alertType = new AlertType(description:"Solicitar al proveedor la gestión del visado (primera etapa)",nameOfEstimatedDateField:"requireVisaFirstStageDate",nameOfCompletionField:"visaFirstStageReceivedDate",alertTerm:20)
				alertType.save(flush:true)
				alertType = new AlertType(description:"Solicitar al proveedor la gestión del visado (segunda etapa)",nameOfEstimatedDateField:"requireVisaSecondStageDate",nameOfCompletionField:"visaSecondStageReceivedDate",alertTerm:0)
				alertType.save(flush:true)
