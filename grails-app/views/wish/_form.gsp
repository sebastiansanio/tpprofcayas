...
<div>
	 ...
	 <p>Cant doc fase 1: ${DocumentType?.list().findAll{it.isPhase1()}.size()}</p> 
	 <p>Cant doc fase 2: ${DocumentType?.list().findAll{it.isPhase2()}.size()}</p>
	 <p>Cant doc total: ${DocumentType?.count()}</p> 
		...
		<div class="control-group fieldcontain ${hasErrors(bean: wishInstance, field: 'firstStageRequiredDocuments', 'error')} ">
			<table class="table table-hover">
				...
				<tbody>
					<g:each var="document" in="${DocumentType?.list().findAll{it.isPhase1()}}">
						<tr>
		                  	<td> 
		                  	...
		                  	<td>	
		                  		<p><a role="button" class="btn btn-small btn-primary"> ${message(code: 'wish.loadFile.label', default: 'Add file')}</a></p>
		                  		<p><a role="button" class="btn btn-small btn-primary"> ${message(code: 'wish.deleteDocument.label', default: 'Delete document')}</a></p>
		                  	 </td>
						</tr>
					</g:each>
											<tr>
		                  	<td> 
			                  	<label class="checkbox" id="chk1" onchange="ver()">
							    	<input type="checkbox"> Tipo de docuemtno1
							    </label>
							</td>
		                  	<td>
		                  		<g:textField name="bill" id="txt1" value="${wishInstance?.bill}" disabled="true"/>
		                  	</td>
		                  	<td>
		                  		<bs:datePicker name="documentDay" id="date1" precision="day"  value="${wishInstance?.billDate}" default="none" noSelection="['': '']" disabled="true" />
							</td>
		                  	<td>	
    	...
    	<script>
    		...
    		function ver(){
            		...
            		{$("#ol").text("hola2");
        				...
        				$("#btnA1").attr("disabled", "false");
        				$("#btnE1").attr("disabled", "false");
                	} else
                    {$("#ol").text("hola3");
        				$("#txt1").attr("disabled", "true");
        				$("#date1").attr("disabled", "true");
        				$("#btnA1").attr("disabled", "true");
        				$("#btnE1").attr("disabled", "true");                        
                    }
        		}
    	</script>
		<!-- 
